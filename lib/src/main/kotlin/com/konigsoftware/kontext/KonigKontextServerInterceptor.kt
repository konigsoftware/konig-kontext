 ...
 */
class KonigKontextServerInterceptor<KontextType>(private vararg val konigKontextKeys: KonigKontextKey<KontextType>) :
    ServerInterceptor {
    override fun <ReqT : Any?, RespT : Any?> interceptCall(
        ...
        var updatedGrpcContext = GrpcContext.current()

        konigKontextKeys.forEach { konigKontextKey ->
            ...
            updatedGrpcContext = updatedGrpcContext.withValue(konigKontextKey.grpcContextKey, konigKontextValue)
        }

        return Contexts.interceptCall(updatedGrpcContext, call, headers, next);
    }
}
